...
const SkillCard = ({ course, onClick, variant = "default" }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-56 overflow-hidden">
        ...
        />
        {course.badge && (
          <div className="absolute top-4 right-4 bg-[#FFC107] text-black  font-bold px-3 py-1 rounded-full">
            {course.badge}
          </div>
        )}
        {isHighlight && (
      ...
      <div className="p-6">
        ...
        <h3
          className={`font-bold text-xl mb-2 ${isHighlight ? "text-[#020B2D]" : "text-[#020B2D]"}`}
        >
          {course.title}
        </h3>

        <p
          className={`text-sm mb-4 line-clamp-2 ${isHighlight ? "text-[#020B2D]/70" : "text-gray-600"}`}
        >
        ...
        <div className="flex flex-col items-start gap-4 justify-between mt-4">
          ...
          <button
            ...
            ${isHighlight ? "bg text-[#020B2D]" : "text-[#020B2D]"}`}
          >
